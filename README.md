...
This documentation includes a ready-to-use learning agent that has been trained for 50,000 episodes with a disruption profile located in `data/raw/`.
...
AI4Resilience-GUI/
  ...
  frontend/            SvelteKit 5 frontend that consumes the Flask API
  notebooks/           exploratory notebooks
  scripts/             operational helper scripts
  templates/           small Flask fallback/handoff page
  tests/               smoke tests
```
...
6. Launch the SvelteKit frontend from `frontend/` with `bun --bun run dev`.
...
### 1. Model input
- Once the repository is cloned, there is a folder called `data/raw/` consisting of default datasets required for running the model.
...
- In `data/processed/kbest/`, the K-best solution for the default demand is already given.
...
- By default, Q-tables are stored in `artifacts/models/q_tables/` and training metrics are stored in `artifacts/metrics/training/`.
...
- The output of the Model Implementation are stored in `artifacts/runs/simulation_outputs/`. Users can upload to files for comparison in the given box and determine the labell
...
### 5. `src/lahso/policy_function.py`
...
### 7. `src/lahso/optimization_module.py`
...
### 9. `src/lahso/simulation_module.py`
...
### 10. `src/lahso/service_to_path.py`
Converts a network of terminals and service lines into possible paths for the optimization algorithm. It calculates the associated costs for each path and writes them to `data/processed/possible_paths/`.

### 11. `src/lahso/kbest.py`
Stores potential solutions for each shipment in a solution pool to minimize the frequency of triggering the optimization module. Uses a rolling horizon approach to update service capacities and provide solutions for varying announcement times, and writes the results to `data/processed/kbest/`.

### 12. `src/lahso/model_train.py` and `src/lahso/model_implementation.py`
...
All required source datasets are located in `data/raw/`. The `Data Structure` file is located at `docs/datasets/data-structure.pdf`. Additional generated datasets include:
- **Possible Paths**: Generated by running `uv run service_to_path` and stored in `data/processed/possible_paths/`.
- **Shipment Request with Solution Pool**: Generated by executing `uv run generate_kbest` and stored in `data/processed/kbest/`.
- **Disruption Sets**: Vary by occurrence probability.

The trained Q-table can be found in `artifacts/models/q_tables/` (for example `q_table_200_50000_eps.pkl`).
...
3. **Generate Paths**:
    If using a new network file, run `uv run service_to_path` to generate possible paths.

4. **Generate Solution Pool**:
    If using the solution pool approach, run `uv run generate_kbest` to store possible solutions for each shipment.

5. **Run the Simulation**:
    Use `uv run ui` to run the Gradio interface, or use the package entrypoints such as `uv run model_train` and `uv run model_implementation`.
...
8. **Extract Observations**:
    For implementation, observations from each simulation episode are saved in `artifacts/runs/simulation_outputs/`. Shipment logs are stored in `artifacts/runs/shipment_logs/` if `extract_shipment_output` is set to `True` in `Config.py`.

---

For more details on how files interact, refer to `docs/architecture/flow-diagram.pdf`.


# Legacy/Local development version
